...
const EditLocationModal = ({ locationmodalVisible, setLocationModalVisible, onDone, selectedLocations, selectedCities }) => {
    ...
    useEffect(() => {
        if (selectedCities && cities.length > 0) {
            const city = cities.find(city => city.Id === selectedCities.Id);
            if (city) {
    ...
    const handleLocationChange = (location, newValue) => {
        const updatedLocations = [...selectedLocation];
        if (newValue) {
            updatedLocations.push(location);
        } else {
            const indexToRemove = updatedLocations.findIndex(loc => loc.Id === location.Id);
            if (indexToRemove !== -1) {
                updatedLocations.splice(indexToRemove, 1);
            }
        }
                        ...
                        {Array.isArray(locations) && locations.map((location, index) => (
                            <View key={index} style={styles.option}>
                                <CheckBox
                                    ...
                                    onValueChange={(newValue) => handleLocationChange(location, newValue)}
